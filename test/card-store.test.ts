...
import { store, addCard, toggleCard, removeCard } from './../src/app/components/card.store';
import { Card } from './../src/app/components/card';

const createMockCard = () => ({ id: '1', title: 'Test Card', body: 'Test Card', isDone: false });
...
describe('Card Store', () => {
    ...
    it('should toggle a card', () => {
        const card = createMockCard();
        store.dispatch(addCard(card));
        store.dispatch(toggleCard(card.id));
        const state = store.getState();
    ...
    it('should remove a card', () => {
        const card = createMockCard();
        store.dispatch(addCard(card));
        store.dispatch(removeCard(card.id));
        const state = store.getState();
    ...
    it('should not toggle a non-existing card', () => {
        ...
        store.dispatch(toggleCard('non-existing-id'));
        const state = store.getState();
    ...
    it('should not remove a non-existing card', () => {
        const initialState = store.getState();
        store.dispatch(removeCard('non-existing-id'));
        const state = store.getState();
        expect(state).toEqual(initialState);
    });
});